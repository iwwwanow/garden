<script lang="ts">
	...
	import { seeds as seedsApi, flowers as flowersApi, users as usersApi, ApiError, type Seed, type FlowerTemplate, type User } from '$lib/api';
	...
	async function resolveUsername() {
		...
		resolving = true; error = '';
		try { resolvedUser = await usersApi.getByUsername(shareUsername.trim()); }
		catch { resolvedUser = null; error = 'Пользователь не найден'; }
	...
	async function share() {
		...
		try {
			...
			showShare = false; shareUsername = ''; resolvedUser = null;
			await load();
		} catch (e: unknown) { error = e instanceof ApiError ? e.message : 'Ошибка передачи'; }
	...
	<div class="page">
		...
		{#if showShare}
			<div>
				...
				<input class="input" id="share-to" type="text" bind:value={shareUsername}
					placeholder="@username" onblur={resolveUsername} />
				{#if resolving}<span>...</span>
				{:else if resolvedUser}<span>✓ {resolvedUser.first_name || resolvedUser.username}</span>
				{/if}
			</div>
			<div>
				<label for="share-qty">Количество (макс. {seed.quantity})</label>
				<input class="input" id="share-qty" type="number" bind:value={shareQty}
					min="1" max={seed.quantity} />
			</div>
