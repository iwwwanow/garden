<script lang="ts">
	...
	import { me, flowers as flowersApi, totalFd, formatDate, type UserFlower, type FlowerTemplate } from '$lib/api';
...
{:else}
	<div class="page">
		...
		<table><tbody>
			...
			<tr><td>Последний полив</td><td>{flower.last_watered_at ? formatDate(flower.last_watered_at) : '—'}</td></tr>
		</tbody></table>

		{#if isPublic}
			<ActionButton variant="muted" onclick={toggleVisibility}>E Скрыть</ActionButton>
		{:else}
			<ActionButton variant="primary" onclick={toggleVisibility}>E Выставить</ActionButton>
		{/if}
	</div>
{/if}
