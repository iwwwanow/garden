<script lang="ts">
	...
	import { me, flowers as flowersApi, totalFd, type UserFlower, type FlowerTemplate } from '$lib/api';
	...
	async function load() {
		loading = true;
		try {
			const [data, tmpl] = await Promise.all([me.get(), flowersApi.listTemplates()]);
...
<div class="page">
	...
	{:else}
		{#if flowers[0]}
			<button onclick={() => goto(`/herbarium/${flowers[0].id}`)} style="display:block; width:100%;">
