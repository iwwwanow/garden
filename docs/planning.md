...
**UI-kit исходники:** `/home/ubuntu-operator/Projects/iwwwanow_ui-kit` — дорабатывается отдельно, в будущем публикуется как npm-пакет.
...
- **Заменить пустой блок-фолбэк на карточках:** сейчас если `imagePath` не передан — карточка показывает серый `var(--LIGHT)` прямоугольник. Нужно либо гарантировать наличие картинки для всех шаблонов, либо добавить SVG/ASCII-плейсхолдер с характером. Фолбэк-emoji удалён намеренно.
